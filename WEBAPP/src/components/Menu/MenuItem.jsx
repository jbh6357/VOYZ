...
const MenuItem = ({ item, selectedLang, reviewViewMode, cart, onAddToCart, onRemoveFromCart, onShowReviews }) => {
    return (
        <div className='menu-item'>
            ...
            >
                ...
                {getItemDescription(item, selectedLang) ? (
                    <div className='item-description'>{getItemDescription(item, selectedLang)}</div>
                ) : (
                    <div className='item-description placeholder'>메뉴 설명 준비중입니다.</div>
                )}
            ...
            <div className='reviews-preview'>
                ...
                {item?.reviews && item.reviews.length > 0 && item.reviews[0] ? (
                    <div className='review-snippet'>
                        ...
                        {getReviewText(item.reviews[0], selectedLang, reviewViewMode)}" -{' '}
                        {item.reviews[0]?.user || '익명'}
                    </div>
                ) : (
                    <div className='review-snippet placeholder'>아직 리뷰가 없습니다 (주문 후 리뷰 작성 가능)</div>
                )}
            </div>
